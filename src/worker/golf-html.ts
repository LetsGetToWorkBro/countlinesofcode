...
${
  entries.length
    ? `<p class="note">Leader: <strong>${num(entries[0]!.code)}</strong> lines of code from
${escapeHtml(`${entries[0]!.owner}/${entries[0]!.repo}`)}, ${num(entries.length)} ${entries.length === 1 ? 'entry' : 'entries'}.</p>`
  ...
  return page(
    'Code Golf: one task, fewest lines wins | 1999.LOC',
    `<h1>Code Golf</h1>
...
is whatever this counter says: same rules for everyone,
...
export function challengePageHtml(board: ChallengeBoard, origin: string): string {
  ...
  return page(
    `${challenge.title} in the fewest lines of code | 1999.LOC`,
...
${siteFooter()}`,
    {
      description: `${challenge.brief} Ranked by lines of code, counted by 1999.LOC. ${

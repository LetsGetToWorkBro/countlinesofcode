/**
 * Monero keys, addresses and seed phrases, bundled to public/monero.js.
 *
 * ## Why this exists at all, and why it argues against itself
 *
 * A wallet generator on a website is a genuinely dangerous thing. Every other
 * tool here fails loudly: a broken GIF encoder produces a broken GIF. This one
 ...
 * and it looks identical to a correct one. Web wallet generators have also been
 * a favourite target for supply-chain attacks, because a single altered script
 * on a single day harvests everything generated that day.
 *
 * So the page it drives spends more words talking a visitor out of trusting it
 * than it does explaining how to use it, and the checks below exist so that
 * "verify it yourself" is a thing somebody can actually do rather than advice.
 *
 * ## What is trusted, and what is checked
 *
 * No cryptography is written here. Keccak-256 comes from @noble/hashes and the
 * ed25519 group arithmetic from @noble/curves, both audited and both vendored
 * from this origin. What is written here is the part that is Monero's rather
 * than cryptography's: scalar reduction, the key derivation, the address
 * encoding, and the seed phrase.
 *
 * Those are checked four ways, and `selfTest()` runs all of them in the
 * visitor's own browser before the page will generate anything:
 *
 ...
 *      publishes it.
 *   3. **A real address, made by the official implementation.** The donation
 *      address on this site was generated by the Monero wallet, not by this
 *      code. Decoding it, re-deriving its checksum and re-encoding it back to
 *      the identical 95 characters exercises base58 both ways and Keccak, and
 *      it is a cross-check against software that is not ours.
 ...
 * proves nothing. That is why the page's last instruction, the one it repeats,
 * is to restore the phrase in the official Monero wallet and confirm it shows
 * the same address **before** anything is sent to it. That check uses an
 * independent implementation, which is the only kind of proof that counts.
 */
 ...
 *
 * Worth being clear about the limit, and the page says it too: a valid
 * checksum proves the address was not corrupted in transit. It does not prove
 * it is the address you were given. Malware that swaps an address on the
 * clipboard swaps in one that checksums perfectly.
 */
...
/**
 * Fold whatever the visitor typed into the browser's randomness.
 *
 * Keccak over both, which is the property worth stating carefully: this cannot
 * be *worse* than the browser's bytes alone, because hashing a uniformly random
 * 32 bytes leaves it uniformly random, and it can be better, because a browser
 ...
 * typing nothing loses nothing.
 *
 * It is not a substitute for a good generator. If `getRandomValues` is broken
 * and the attacker also knows what you typed, this saves nobody.
 */
export function mixEntropy(browser: Uint8Array, extra: Uint8Array = new Uint8Array(0)): Uint8Array {
  if (browser.length !== 32) throw new Error('The browser must supply 32 bytes.');
  if (!extra.length) return keccak_256(browser);
...
/** The site's donation address. Made by the official Monero wallet, not by this code. */
...
/**
 * Every check, run in the visitor's browser.
 *
 * The page refuses to generate anything unless all of them pass. A tampered or
 * broken build fails here rather than at the point where somebody has already
 * sent money to an address nobody holds the key for.
 */
export function selfTest(): Check[] {
  ...
  add('A real address, read back', 'Base58 and the checksum agree with the official Monero wallet, which made this address.', () => {
    ...
    return [parsed.valid && parsed.network === 'mainnet' && parsed.kind === 'subaddress',
...
const globalScope = globalThis as unknown as { LOC1999_XMR?: Record<string, unknown> };
globalScope.LOC1999_XMR = {
  toHex,
  fromHex,
  reduceScalar,
  publicFromSecret,

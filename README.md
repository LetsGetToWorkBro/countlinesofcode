...
| Tool | URL | What it does | Runs on |
|---|---|---|---|
| **Wallets** | `/wallet.html` | *Monero* tab: a full hot wallet in the tab — create, restore, receive, send; keys never leave the browser, node traffic proxied through `/api/xmr`. *Bitcoin* tab: the same deal for BTC — BIP84 (12 words, bc1 addresses), created/restored/watch-only, signed in the tab, with balance and broadcast through `/api/btc` to an Esplora explorer (mempool.space or blockstream.info, or your own). *Check an address* tab: validate an address, restore a seed, or generate a paper wallet offline, verified against published test vectors. *Vault* tab: turn an `unsigned_monero_tx` written by Feather or the Monero GUI into QR frames an offline [Labyrinth Vault](https://labyrinth.vision) can read; the vault opens it and says what the payment claims, and does not sign it. Holds no keys, uploads nothing. | wallets: browser + **node/explorer proxy** · checker, vault frames: browser |
| **Swap** | `/swap.html` | Monero in or out, against BTC, USDT (Tron or Ethereum), ETH and USDC (Ethereum or Solana), through instant exchanges. Every pair has XMR on one side; the payout address is checked against the chain it is being paid to. Quotes from Exolix and Godex out of the box (neither needs a key, so the board compares rather than reporting one price), plus ChangeNOW (`CHANGENOW_API_KEY`) and Trocador (`TROCADOR_API_KEY`, an aggregator that quotes the best of about fifteen no-KYC desks) when those secrets are set; the best quote is selected automatically and any desk can be picked instead; the Worker relays via `/api/swap` so the exchange sees Cloudflare, not the visitor, and stores nothing about the order. Trocador's adapter has not been run against the live API (they gate the docs behind the key too), so check one real quote and one real order before leaving that secret set; it refuses any deposit address that is not on the chain being sent, so a misread field fails closed rather than printing an address to pay into. | **the server** (relay only, no custody, no storage) |

...
| **Swap** | `/swap.html` | BTC or USDC (ERC-20 or Solana) in, Monero out, through instant exchanges. Quotes from Exolix out of the box, plus ChangeNOW when a `CHANGENOW_API_KEY` secret is set; the Worker relays via `/api/swap` so the exchange sees Cloudflare, not the visitor, and stores nothing about the order. | **the server** (relay only, no custody, no storage) |

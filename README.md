...
| **Swap** | `/swap.html` | Monero in or out, against BTC, USDT (Tron or Ethereum), ETH and USDC (Ethereum or Solana), through instant exchanges. Every pair has XMR on one side; the payout address is checked against the chain it is being paid to. Quotes from Exolix and Godex out of the box (neither needs a key, so the board compares rather than reporting one price), plus ChangeNOW when a `CHANGENOW_API_KEY` secret is set; the best quote is selected automatically and any desk can be picked instead; the Worker relays via `/api/swap` so the exchange sees Cloudflare, not the visitor, and stores nothing about the order. | **the server** (relay only, no custody, no storage) |
